...
class GraphormerDGLDataset(Dataset):
    ...
    def __extract_edge_and_node_features(
        ...
        def extract_tensor_from_node_or_edge_data(
            ...
            for feature_or_dict in feature_dict.values():
                # print(feature_or_dict, "a")
                feature_or_dict = feature_or_dict.data.float()
                # print(feature_or_dict)
                # print(type(feature_or_dict), 'THIS IS ME LOL')
        ...
        # node_int_feature = torch.tensor(node_float_feature, dtype=int).squeeze(1)

        node_int_feature = torch.tensor(node_int_feature, dtype=float).squeeze(1)
    ...
    def __preprocess_dgl_graph(
        self, graph_data: DGLGraph, y: torch.Tensor, idx: int,smiles
    ) -> PYGGraph:
        ...
        #print("INSIDE PREPROCESS DGL GRAPH")
        #print(graph_data.ndata['h'])
        (
            node_int_feature,
            node_float_feature,
            edge_int_feature,
        ...
        edge_int_feature = edge_float_feature.long()
        ...
        pyg_graph.x = torch.squeeze(node_float_feature, dim=1)
        #print('SHAPE', pyg_graph.x.shape)
    ...
    def __getitem__(self, idx):
        if isinstance(idx, int):
            ...
            graph, y, smiles = self.dataset[idx]
            return self.__preprocess_dgl_graph(graph, y, idx, smiles)
        else:
